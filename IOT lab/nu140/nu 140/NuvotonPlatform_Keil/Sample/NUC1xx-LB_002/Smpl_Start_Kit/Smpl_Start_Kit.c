...
int main(void)
{
	...
	//printf("Testing SEMIHOST debug mode\n");

	print_lcd(0, "Welcome! TPCU   ");	  
	print_lcd(1, "CSIE Department ");
	print_lcd(2, "Cortex-M0 MCU   ");	  
	print_lcd(3, "Nuvoton NUC140 !");	  	  
	Timer_initial();
	
	while(1)
	{
		//printf("7-segment LED loop\n");
		for(i=0;i<9;i++)
		{
		  for(j=0;j<4;j++)
		  {
